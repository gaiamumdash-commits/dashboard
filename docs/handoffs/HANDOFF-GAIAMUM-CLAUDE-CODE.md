...
- **Migrations `0002_colaboracao_equipe.sql` e `0003_permissao_por_quadro.sql` aplicadas com sucesso no banco** (2026-08-30, coladas manualmente no SQL Editor pelo Fabio — confirmado via query de diagnóstico que as 4 tabelas novas, `data_inicio`, `escopo` e `convites.projeto_id` existem). **Falta ainda testar o fluxo de convite de ponta a ponta** (criar em `/equipe`, aceitar em `/convite/[token]` com uma segunda conta) antes de considerar essa onda validada — só a migration foi verificada, não o fluxo pela UI.
