...
## Estado confirmado (2026-08-30, madrugada/manhã — última atualização)

- **⚠️ AÇÃO IMEDIATA AO RETOMAR:** existem 2 migrations novas **escritas mas NÃO aplicadas no banco** — `supabase/migrations/0002_colaboracao_equipe.sql` e `0003_permissao_por_quadro.sql`. Sem elas, o código novo desta sessão (equipe, convites, checklist, permissão por quadro) vai dar erro em produção (tabelas/colunas não existem ainda). Aplicar as duas, **nessa ordem**, colando o conteúdo no SQL Editor do Supabase (`https://supabase.com/dashboard/project/zfjtcivusdmjvdbycpjs/sql/new`) — o Claude Code tem um bloqueio de segurança que impede aplicar via CLI/API (proposital, não contornar). Depois de aplicar, testar o fluxo de convite (criar convite em `/equipe`, aceitar em `/convite/[token]`) antes de seguir construindo em cima.
- **Trabalho de código desta sessão ainda incompleto** (compila e passa lint, mas falta UI): `FormularioConvite` ainda não tem o seletor "convidar pra qual quadro" (só convida pro workspace inteiro por enquanto); `ListaMembros`/`ListaConvites` não mostram a que projeto um convite pertence; os botões de excluir projeto/tarefa não escondem visualmente de quem não é gestor (a RLS já bloqueia no banco, é só a UI que ainda deixa o botão visível pra todo mundo — não é falha de segurança, é só polimento de UX pendente).
