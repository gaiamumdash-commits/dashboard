...
- **Módulo Financeiro (Onda 3) — primeira versão construída, migrada em produção e testada de ponta a ponta via Playwright real.** Plano completo em `C:\Users\proff\.claude\plans\valiant-zooming-sutton.md`. Rota nova `/financeiro` (+ `/financeiro/importar`), **restrita a owner** (dado sensível — pagamento, extrato bancário; redirect pra `/projetos` pra qualquer outro papel, mesmo padrão RLS `current_papel(tenant_id) = 'owner'` já usado no kanban). Migration `0008_modulo_financeiro.sql` (tabelas `contas_fixas_modelo`, `contas_a_pagar`, `anexos`, `contas_categoria_regras` + bucket privado `anexos` no Supabase Storage).
  ...
  - **Anexo de comprovante** — upload direto no cartão da conta (bucket privado, link sempre via signed URL de 10min, nunca público). Peça desenhada pra ser reaproveitada — `anexos.entidade_tipo` já aceita `'tarefa'` além de `'conta_a_pagar'`, só falta a policy de RLS e a UI no cartão de tarefa (ver bullet "Melhorar o cartão de tarefa" abaixo).
...
- **Melhorar o cartão de tarefa (kanban) — pedido do Fabio em 2026-08-30 (sessão do Módulo Financeiro), fazer logo depois de fechar o financeiro.** Testei o cartão ao vivo: abrir o cartão, marcar membro responsável e usar o checklist já funcionam (confirmado nesta sessão, sem bug). Dois gaps reais que ele quer resolvidos:
  1. **Anexo de arquivo.** Regra que ele deu: arquivo pequeno anexa direto no cartão; arquivo grande, linka com Google Drive em vez de fazer upload. A peça técnica já existe — é a mesma tabela `anexos` (+ bucket `anexos` no Supabase Storage) construída pro comprovante de conta a pagar no Módulo Financeiro, que já reserva `entidade_tipo = 'tarefa'` no schema (`supabase/migrations/0008_modulo_financeiro.sql`) só sem policy de RLS nem UI ainda — falta adicionar a policy (`tem_acesso_ao_projeto`, não owner-only como o financeiro) e plugar o componente de upload no `DetalheTarefa`.
  2. **Prioridade não é editável depois que o cartão é criado** — hoje `FormularioNovaTarefa` pede prioridade/tag/prazo tudo de uma vez na criação, e o `DetalheTarefa` (modal do cartão) não tem campo pra mudar prioridade depois. O Fabio quer o fluxo estilo Trello: criar o cartão rápido só com o título, e evoluir tudo (prioridade, responsável, checklist, anexo) depois clicando no próprio cartão — "se eu não tenho essas opções depois, perco flexibilidade pra gerenciar o projeto".
