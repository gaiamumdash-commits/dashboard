...
- **Nota registrada, não implementar ainda:** Fabio quer conectar o **Nibo Empresas via API REST** pro futuro Módulo Financeiro (Onda 3, ainda não iniciado) — ele mesmo providencia a credencial da API quando chegar a hora; só guardar a intenção pra já pensar a integração desde o desenho do schema financeiro.
...
4. **Fazer push dos commits locais acumulados** (bugs de convite corrigidos + notificação por e-mail) — pedir aprovação do Fabio antes, como sempre (ver "Convenção de push").
5. Reconferir certificado SSL da Vercel se ainda não foi feito: `curl -I https://www.gaiamum.com.br`.
6. **Decidir com o Fabio:** apagar os usuários de teste (`gaiamumdash@gmail.com` já é a conta real do Fabio, mantém; `gaiamumdash+convite@gmail.com` foi só teste desta sessão) ou manter como conta de teste recorrente.
