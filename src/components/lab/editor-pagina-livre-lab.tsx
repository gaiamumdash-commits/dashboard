...
import {
  atualizarConteudoPaginaLivreLab,
  excluirPaginaLivreLab,
  renomearPaginaLivreLab,
} from "@/lib/ecc/lab/paginas-livres";
...
/** Espelha src/components/projetos/editor-pagina-livre.tsx, usando as Server
 * Actions Lab-aware. Excluir sempre navega pra /lab/paginas — sem prop
 * `podeExcluir` (tenant do Lab é sempre solo-owner). */
export function EditorPaginaLivreLab({
  paginaId,
  projetoId,
  tituloInicial,
  conteudoInicial,
  ...
  function salvarAgora() {
    ...
    setStatus("salvando");
    atualizarConteudoPaginaLivreLab(paginaId, projetoId, editor.document, atualizadoEmRef.current)
      .then((resultado) => {
  ...
  function aoSairDoTitulo() {
    ...
    renomearPaginaLivreLab(paginaId, projetoId, tituloLimpo).catch((err) => {
  ...
  function excluir() {
    ...
    excluirPaginaLivreLab(paginaId, projetoId)
      .then(() => router.push("/lab/paginas"))
