...
import { criarDecisaoLab, editarDecisaoLab, excluirDecisaoLab } from "@/lib/ecc/lab/decisoes-indicadores";
...
export function ListaDecisoesLab({
  ...
  function salvar(formData: FormData) {
    iniciarTransicao(async () => {
      try {
        if (emEdicao) {
          await editarDecisaoLab(emEdicao.id, projetoId, formData);
        } else {
          await criarDecisaoLab(projetoId, formData);
        }
        fecharDialog();
        router.refresh();
      } catch (err) {
        toast.error(mensagemDeErro(err, "Falha ao salvar decisão."));
      }
    });
  }
  ...
  function excluir(decisao: Decisao) {
    ...
    iniciarTransicao(async () => {
      try {
        await excluirDecisaoLab(decisao.id, projetoId);
        router.refresh();
      } catch (err) {
        toast.error(mensagemDeErro(err, "Falha ao excluir decisão."));
      }
    });
  }
