...
import { DetalheItemAgendaLab } from "@/components/lab/detalhe-item-agenda-lab";
...
/** Duplicata de GradeSemanal (components/agenda/grade-semanal.tsx) — único
 * diff é renderizar DetalheItemAgendaLab em vez de DetalheItemAgenda. */
export function GradeSemanalLab({ itens, chaveSemana }: { itens: ItemAgenda[]; chaveSemana: string }) {
  ...
  return (
    <div>
      ...
      {itemSelecionado && (
        <DetalheItemAgendaLab key={itemSelecionado.id} item={itemSelecionado} aoFechar={() => setItemSelecionado(null)} />
      )}
    </div>
  );
}
