...
import { atualizarValorIndicador, criarIndicador, editarIndicador, excluirIndicador } from "@/lib/ecc/indicadores";
...
export function ListaIndicadores({
  ...
  function salvar(formData: FormData) {
    iniciarTransicao(async () => {
      if (emEdicao) {
        await editarIndicador(emEdicao.id, projetoId, formData);
      } else {
        await criarIndicador(projetoId, formData);
      }
      fecharDialog();
      router.refresh();
    });
  ...
  function excluir(indicador: Indicador) {
    ...
    iniciarTransicao(async () => {
      await excluirIndicador(indicador.id, projetoId);
      router.refresh();
    });
  }
  ...
  function salvarValorAtual(indicador: Indicador, valor: string) {
    ...
    iniciarTransicao(async () => {
      await atualizarValorIndicador(indicador.id, projetoId, valorNumerico);
      router.refresh();
    });
  }
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-3">
        {indicadoresIniciais.map((indicador) => {
          const percentual = indicador.meta > 0 ? Math.min(100, Math.round((indicador.valor_atual / indicador.meta) * 100)) : 0;
          return (
