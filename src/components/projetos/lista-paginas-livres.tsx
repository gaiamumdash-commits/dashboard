...
import { criarPaginaLivre, excluirPaginaLivre } from "@/lib/ecc/paginas-livres";
import { mensagemDeErro } from "@/lib/erro-cliente";
import { Dialog } from "@/components/ui/dialog";

export function ListaPaginasLivres({
  projetoId,
  paginasIniciais,
  podeExcluir,
}: {
  ...
  function criar(formData: FormData) {
    ...
    iniciarTransicao(async () => {
      try {
        const nova = await criarPaginaLivre(projetoId, titulo);
        setCriando(false);
        router.push(`/projetos/${projetoId}/paginas/${nova.id}`);
      } catch (err) {
  ...
  function excluir(pagina: PaginaLivre) {
    ...
    iniciarTransicao(async () => {
      try {
        await excluirPaginaLivre(pagina.id, projetoId);
        router.refresh();
      } catch (err) {
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-3">
        {paginasIniciais.map((pagina) => (
          ...
          >
            <Link href={`/projetos/${projetoId}/paginas/${pagina.id}`} className="flex-1">
